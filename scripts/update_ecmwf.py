...
def latest_candidate_run(now):
    # ECMWF IFS has 4 deterministic runs per day: 00/06/12/18 UTC.
    # Open-Meteo's Single Runs API exposes the exact initialisation run.
    base = now.replace(minute=0, second=0, microsecond=0)
    hour = base.hour - (base.hour % 6)
    candidate = base.replace(hour=hour)
    if (now - candidate) < timedelta(hours=4, minutes=30):
