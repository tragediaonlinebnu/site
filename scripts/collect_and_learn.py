...
async def main():
    ...
    if current is not None:
        for h in ('2','4','6','12','24'):
            hydro=hydro_component_from_memory(response_state,now,r,current,slope,h); p,tcomp,rcomp,fcomp,dcomp,hcomp=project(current,rainp[h],slope,h,params,future,dam_now,hydro); pred[h]=round(p,4); components[h]={'trend':tcomp,'observed_rain':rcomp,'forecast_rain':fcomp,'dam':dcomp,'hydro':hcomp}
