...
def _update_city_influence(table, event):
    """Atualiza associação histórica cidade -> resposta do rio.

    A resposta do rio é compartilhada por toda a bacia; portanto este campo
    representa *associação/atribuição hidrológica observada*, não causalidade
    física isolada. A confiança cresce apenas com eventos e consistência.
    """
    spatial=(event.get('spatial_rain') or {}).get('cities') or {}
    if not spatial: return table
    total=sum(_city_window(row) for row in spatial.values())
    if total<=0: return table
    rise=max(0.0,_num(event.get('rise_cm')))
    delay=_num(event.get('delay_hours'))
    for city,row in spatial.items():
        mm=_city_window(row)
        if mm<=0: continue
        rec=table.setdefault(city,{
            'events':0,'sum_mm':0.0,'sum_rise_cm':0.0,'sum_allocated_rise_cm':0.0,
            'sum_delay_hours':0.0,'samples':[]
        })
        rec['events']=int(rec.get('events',0) or 0)+1
        rec['sum_mm']=_num(rec.get('sum_mm'))+mm
        rec['sum_rise_cm']=_num(rec.get('sum_rise_cm'))+rise
        # Alocação proporcional explícita: proxy interpretável, nunca tratada
        # como relação causal exata.
        allocated=rise*(mm/total)
        rec['sum_allocated_rise_cm']=_num(rec.get('sum_allocated_rise_cm'))+allocated
        rec['sum_delay_hours']=_num(rec.get('sum_delay_hours'))+delay
        samples=list(rec.get('samples') or [])
        samples.append({'mm':round(mm,3),'rise_cm':round(rise,3),'allocated_rise_cm':round(allocated,3),'delay_hours':round(delay,3)})
        rec['samples']=samples[-100:]
        n=rec['events']
        rec['mean_mm']=round(rec['sum_mm']/n,3)
        rec['mean_rise_cm']=round(rec['sum_rise_cm']/n,3)
        rec['mean_allocated_rise_cm']=round(rec['sum_allocated_rise_cm']/n,3)
        rec['mean_delay_hours']=round(rec['sum_delay_hours']/n,3)
        rec['allocated_cm_per_mm']=round(rec['sum_allocated_rise_cm']/max(1e-9,rec['sum_mm']),5)
        # Confiança depende de quantidade e estabilidade da amostra.
        vals=[_num(s.get('allocated_rise_cm')) for s in rec['samples']]
        mean=sum(vals)/len(vals) if vals else 0.0
        if len(vals)>1:
            spread=sum(abs(v-mean) for v in vals)/len(vals)
            consistency=1.0/(1.0+spread/max(1.0,abs(mean)))
        else:
            consistency=0.25
        sample_conf=min(1.0,n/12.0)
        conf=sample_conf*consistency
        rec['confidence']=round(conf,3)
        rec['confidence_level']='low' if conf<0.35 else ('medium' if conf<0.7 else 'high')
        rec['interpretation']='associacao_historica; nao_causalidade_isolada'
    return table
    ...
    if isinstance(active,dict) and current is not None:
        ...
        if response_at is not None:
            ...
            if response_age_h>=12.0:
                ...
                # Aprende associação histórica por cidade, com confiança.
                state['city_influence']=_update_city_influence(
                    dict(state.get('city_influence') or {}), event
                )
...
async def main():
    ...
    if current is not None:
        for h in ('2','4','6','12','24'):
            ...
            hydro=hydro_base+hydro_spatial
            p,tcomp,rcomp,fcomp,dcomp,hcomp=project(current,rainp[h],slope,h,params,future,dam_now,hydro)
            pred[h]=round(p,4)
            components[h]={'trend':tcomp,'observed_rain':rcomp,'forecast_rain':fcomp,'dam':dcomp,'hydro':hcomp,'hydro_pattern':hydro_base,'hydro_spatial':hydro_spatial}
