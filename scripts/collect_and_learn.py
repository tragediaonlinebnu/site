...
def update_rain_response(state, now, current, rain, slope, spatial=None):
    ...
    if isinstance(active,dict) and current is not None:
        ...
        if float(current) > previous_max + 0.002:
        ...
        if response_at is not None:
            ...
            # Normalmente observa 12h, mas pode encerrar antes quando o pico já
            # estabilizou. Isso evita perder episódios seguintes durante longas
            # recessões após uma resposta já bem caracterizada.
            ...
            if response_age_h>=12.0 or (rain_finished and enough_curve and stable_h>=3.0):
                event={
