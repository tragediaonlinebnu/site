...
from hydrology import update_events, summarize_events
...
EVENTS=os.path.join(DATA_DIR,'hydrology_events.json')
HYDRO_STATUS=os.path.join(DATA_DIR,'hydrology_status.json')
FORECAST=os.path.join(DATA_DIR,'rain_forecast.json')
    ...
    qualle_meteorologia {
      ...
      data {
        ...
        chuva { acumulado {
          ...
          h072 { value show { value } unit { value } }
...
def rain_quality(stations):
    out={}
    for k in ('h003','h006','h012','h024','h048','h072','h096'):
        by_city={}
        station_count=0
...
def nearest_actual(history,due):
    cand=[x for x in history if abs(x['t']-due)<=20*60*1000]; return min(cand,key=lambda x:abs(x['t']-due)) if cand else None

def learn(params,snaps,history):
    total=0; abs_err={h:[] for h in ('2','4','6','12','24')}; signed_err={h:[] for h in ('2','4','6','12','24')}; now=int(datetime.now(timezone.utc).timestamp()*1000); remaining=[]
    for s in snaps:
        for h in ('2','4','6','12','24'):
            ...
            err=pred-a['level']; abs_err[h].append(abs(err)); signed_err[h].append(err); total+=1; comps=s.get('components',{}).get(h,{})
            ...
            adj('trend_gain',comps.get('trend',0)); adj('rain_gain',comps.get('observed_rain',0)); adj('forecast_rain_gain',comps.get('forecast_rain',0),0.0,1.60); adj('dam_gain',comps.get('dam',0),.20,2.00)
            s[key]=a['level']; s['error_'+h]=err; s['resolved_'+h]=datetime.now(timezone.utc).isoformat()
        if now-s['t']<120*24*3600000:remaining.append(s)
    ...
    for h in ('2','4','6','12','24'):
        if abs_err[h]:
            ...
            params['metrics'].setdefault('rmse_m',{})[h]=round(math.sqrt(sum(x*x for x in signed_err[h])/len(signed_err[h])),4)
            params['metrics'].setdefault('bias_m',{})[h]=round(sum(signed_err[h])/len(signed_err[h]),4)
...
def fetch_forecast():
    # Mesma grade espacial do painel, mas processada no workflow para que o motor
    # também possa usar chuva futura. Se a API falhar, o modelo continua sem ela.
    points=[
      (-26.82,-49.39),(-26.92,-49.64),(-27.02,-49.29),(-27.08,-49.55),(-27.18,-49.78),(-27.22,-49.48),(-27.30,-49.70),(-27.34,-49.38),(-27.42,-49.64),(-27.48,-49.50),(-27.55,-49.72),(-27.58,-49.32),
      (-26.62,-49.38),(-26.65,-49.17),(-26.70,-49.03),(-26.75,-49.29),(-26.78,-49.12),(-26.82,-49.42),(-26.86,-49.27),(-26.89,-49.10),(-26.93,-49.34),(-26.96,-49.19),(-27.00,-49.06),(-27.03,-48.92)
    ]
    try:
        url='https://api.open-meteo.com/v1/forecast?latitude='+','.join(str(x[0]) for x in points)+'&longitude='+','.join(str(x[1]) for x in points)+'&hourly=precipitation&models=ecmwf_ifs025&forecast_days=3&timezone=America%2FSao_Paulo'
        r=requests.get(url,timeout=25); r.raise_for_status(); data=r.json(); rows=data if isinstance(data,list) else [data]
        now=datetime.now(ZoneInfo('America/Sao_Paulo')); sums={6:[],12:[],24:[]}
        for d in rows:
            times=d.get('hourly',{}).get('time',[]); rainv=d.get('hourly',{}).get('precipitation',[]); acc={6:0,12:0,24:0}
            for t,v in zip(times,rainv):
                try: dt=datetime.fromisoformat(str(t)); valf=float(v)
                except Exception: continue
                if dt.tzinfo is None: dt=dt.replace(tzinfo=ZoneInfo('America/Sao_Paulo'))
                dh=(dt-now).total_seconds()/3600
                if 0<=dh<6: acc[6]+=valf
                if 0<=dh<12: acc[12]+=valf
                if 0<=dh<24: acc[24]+=valf
            for h in acc:sums[h].append(acc[h])
        out={str(h):round(sum(v)/len(v),2) if v else None for h,v in sums.items()}
        savej(FORECAST,{'updated_at':datetime.now(timezone.utc).isoformat(),'model':'ECMWF IFS via Open-Meteo','rain_mm':out})
        return out
    except Exception as e:
        print('Falha previsão ECMWF:',type(e).__name__,e)
        return {}
...
async def main():
    ...
    required=('h003','h006','h012','h024','h048','h072','h096')
    ...
    rainp={h:weighted_observed_rain(r['h003'],r['h006'],r['h012'],r['h024'],r['h048'],r['h072'],r['h096'],horizon=int(h)) for h in ('2','4','6','12','24')}
    ...
    events=loadj(EVENTS,[]); events=events if isinstance(events,list) else []
    events=update_events(events,snaps,history,r,current,now)
    event_summary=summarize_events(events)
    pred={}; components={}
    if current is not None:
        for h in ('2','4','6','12','24'):
            future_rain=forecast.get(h) if isinstance(forecast,dict) else None
            p,tcomp,rcomp,fcomp,dcomp,analog,confidence,analog_meta=project(current,rainp[h],slope,h,params,future_rain=None,dams=dam_now,events=events,rain48=r.get('h048'))
            pred[h]=round(p,4); components[h]={'trend':tcomp,'observed_rain':rcomp,'forecast_rain':fcomp,'dam':dcomp,'historical_analog':analog,'analog_confidence':confidence,'analog_meta':analog_meta}
    ...
    savej(SNAP,snaps)
    savej(EVENTS,events[-250:])
    savej(os.path.join(DATA_DIR,'learning_status.json'),{'version':10,'updated_at':params['updated_at'],'evaluated':params['metrics']['evaluated'],'mae_m':params['metrics']['mae_m'],'rmse_m':params['metrics'].get('rmse_m',{}),'bias_m':params['metrics'].get('bias_m',{}),'trend_gain':params['trend_gain'],'rain_gain':params['rain_gain'],'forecast_rain_gain':params['forecast_rain_gain'],'dam_gain':params['dam_gain'],'damping':params['damping'],'rain_quality':quality,'events':event_summary})
    savej(HYDRO_STATUS,{'version':1,'updated_at':datetime.now(timezone.utc).isoformat(),'current_level_m':current,'slope_m_h':slope,'rain_observed_mm':r,'rain_forecast_mm':forecast,'events':event_summary,'event_active':next((e for e in reversed(events) if e.get('status')=='active'),None),'predictions':pred,'prediction_components':components,'model':'hydrological: spatial observed rainfall from WebSocket pluviometers + river trend + dam signal + historical analogs'})
    print(json.dumps({'current':current,'rain':r,'rain_quality':quality,'future_rain':forecast,'dams':dam_now,'slope':slope,'pred':pred,'events':event_summary,'evaluated':params['metrics']['evaluated']},ensure_ascii=False))
