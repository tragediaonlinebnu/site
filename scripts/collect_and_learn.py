...
def station_city(s):
    name=s.get('name') or {}; pos=s.get('position') or {}; rio=((s.get('data') or {}).get('rio') or {})
    parts=[s.get('codigo'),name.get('local'),name.get('general'),name.get('prefix'),pos.get('regiao'),pos.get('bacia'),(rio.get('rio_nome') or {}).get('value')]
    h=norm(' '.join(text(x) for x in parts))
    for city,pat in CITY_PATTERNS:
        if pat.search(h): return city
    return None
...
async def main():
    ...
    damhist=[x for x in damhist if int(x.get('t',-1))!=now]; damhist.append({'model_version':7,'t':now,'dams':dams,'signal':dam_now}); savej(DAM_HIST,damhist[-3000:])
