...
def _rebuild_city_influence(history):
    """Aprende associação multievento por cidade, sem dividir a subida proporcionalmente.

    Usa regressão ridge em eventos concluídos. O coeficiente de uma cidade é
    estimado enquanto as demais cidades também estão no modelo, reduzindo o
    erro de simplesmente atribuir a mesma subida a todas as cidades. Ainda é
    associação estatística, não prova causalidade hidráulica isolada.
    """
    rows=[]; counts={}
    for ev in history or []:
        ...
        rows.append((vals,y,_num(ev.get('delay_hours'))))
        for c in vals: counts[c]=counts.get(c,0)+1
    # Só entram cidades com repetição suficiente; limita dimensão para não
    # "aprender" mais parâmetros do que eventos disponíveis.
    candidates=[c for c,n in counts.items() if n>=6]
    candidates.sort(key=lambda c:(-counts[c],c))
    if not candidates:
        return {'method':'multievent_ridge','events':len(rows),'ready':False,'reason':'insufficient_repeated_city_samples','cities':{}}
    max_features=max(1,min(8,(len(rows)//2)-1))
    features=candidates[:max_features]
    min_events=max(12,2*(len(features)+1))
    if len(rows)<min_events:
        return {'method':'multievent_ridge','events':len(rows),'ready':False,'reason':'insufficient_events_for_multivariate_fit','required_events':min_events,'features':features,'cities':{}}
    xs=[[vals.get(c,0.0) for c in features] for vals,_,_ in rows]
    ys=[y for _,y,_ in rows]
    means=[sum(row[j] for row in xs)/len(xs) for j in range(len(features))]
    stds=[]
    for j,m in enumerate(means):
        v=sum((row[j]-m)**2 for row in xs)/len(xs)
        stds.append(max(1e-6,math.sqrt(v)))
    ymean=sum(ys)/len(ys)
    z=[[ (row[j]-means[j])/stds[j] for j in range(len(features))] for row in xs]
    # intercept + features padronizadas; ridge somente nos coeficientes.
    p=len(features)+1
    ...
    for row,y in zip(z,ys):
        ...
        for i in range(p):
            xty[i]+=q[i]*(y-ymean)
            for j in range(p): xtx[i][j]+=q[i]*q[j]
    ridge=1.0
    for j in range(1,p): xtx[j][j]+=ridge
    beta=_solve_linear_system(xtx,xty)
    if beta is None:
        return {'method':'multievent_ridge','events':len(rows),'ready':False,'reason':'singular_fit','cities':{}}
    coeff=[beta[j+1]/stds[j] for j in range(len(features))]
    preds=[]
    for row in xs:
        preds.append(ymean+sum(coeff[j]*(row[j]-means[j]) for j in range(len(features))))
    sse=sum((y-pred)**2 for y,pred in zip(ys,preds)); sst=sum((y-ymean)**2 for y in ys)
    r2=max(0.0,1.0-sse/sst) if sst>1e-9 else 0.0
    fit_conf=min(1.0,len(rows)/30.0)*max(0.0,min(1.0,r2))
    cities={}
    for j,c in enumerate(features):
        delay_vals=[d for vals,_,d in rows if vals.get(c,0)>0]
        cities[c]={
            'events':counts[c], 'coefficient_cm_per_mm':round(coeff[j],6),
            'mean_mm':round(means[j],4), 'mean_delay_hours':round(sum(delay_vals)/len(delay_vals),3) if delay_vals else None,
            'confidence':round(fit_conf*min(1.0,counts[c]/12.0),3),
            'interpretation':'associacao_multievento_controlando_outras_cidades; nao_causalidade_isolada'
        }
    return {'method':'multievent_ridge','events':len(rows),'ready':True,'features':features,
            'r2':round(r2,4),'confidence':round(fit_conf,3),'cities':cities}
...
def update_rain_response(state, now, current, rain, slope, spatial=None):
    ...
    if isinstance(active,dict) and current is not None:
        # A chuva e a resposta do rio são fases diferentes do mesmo episódio.
        # Quando as janelas curtas caem abaixo do limiar, marcamos o FIM da
        # chuva, mas mantemos o episódio aberto para medir o atraso do rio.
        if raining_now:
            active['last_rain_active_at']=now
            active['rain_ended_at']=None
        elif active.get('rain_ended_at') is None:
            active['rain_ended_at']=now
            state['last_rain_episode']={
                'started_at':active.get('rain_started_at',active.get('started_at')),
                'ended_at':now,
                'duration_hours':round(max(0.0,(now-int(active.get('rain_started_at',active.get('started_at',now))))/3600000.0),3),
        ...
        if active.get('rain_started_at') is not None:
            end_for_duration = now if active.get('rain_ended_at') is None else int(active.get('rain_ended_at'))
