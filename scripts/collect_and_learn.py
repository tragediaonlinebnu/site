...
def hydrological_reference_component(state, now, current, horizon):
    ...
    # O prior é forte como referência, mas limitado por horizonte para não
    # dominar o modelo enquanto ainda há poucos eventos reais.
    cap={'2':0.10,'4':0.20,'6':0.32,'12':0.55,'24':0.90}[str(horizon)]
    return round(min(cap,max(0.0,remaining_cm/100.0)),6)
...
def hydro_component_from_memory(state, now, rain, current, slope, horizon):
    ...
    return round(min({'2':0.10,'4':0.20,'6':0.32,'12':0.55,'24':0.90}[str(horizon)],max(0.0,blended)),6)
