...
async def main():
    ...
    if current is not None:
        for h in ('2','4','6','12','24'):
            ...
            p,tcomp,rcomp,fcomp,dcomp,analog,confidence,analog_meta=project(current,rainp[h],slope,h,params,future_rain=future_rain,dams=dam_now,events=events,rain48=r.get('h048'))
    ...
    savej(HYDRO_STATUS,{'version':1,'updated_at':datetime.now(timezone.utc).isoformat(),'current_level_m':current,'slope_m_h':slope,'rain_observed_mm':r,'rain_forecast_mm':forecast,'events':event_summary,'event_active':next((e for e in reversed(events) if e.get('status')=='active'),None),'predictions':pred,'prediction_components':components,'model':'hybrid trend + observed rainfall + ECMWF + dam signal + historical analogs'})
