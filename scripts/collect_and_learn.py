...
def fetch_ecmwf():
 points=[(lat,lon,reg) for reg,arr in POINTS.items() for lat,lon in arr]
 url='https://api.open-meteo.com/v1/forecast?latitude='+','.join(str(x[0]) for x in points)+'&longitude='+','.join(str(x[1]) for x in points)+'&hourly=precipitation&models=ecmwf_ifs025&forecast_days=2&timezone=America%2FSao_Paulo'
 try:
  r=requests.get(url,timeout=30); r.raise_for_status(); rows=r.json(); rows=rows if isinstance(rows,list) else [rows]
  now=datetime.now(ZoneInfo('America/Sao_Paulo')); out={'6':0.0,'12':0.0,'24':0.0}; counts={k:0 for k in out}
  sector={k:{h:[] for h in out} for k in ('alto','medio')}
  for p,row in zip(points,rows):
   times=row.get('hourly',{}).get('time',[]); rainv=row.get('hourly',{}).get('precipitation',[]); sums={h:0.0 for h in out}
   for ts,v in zip(times,rainv):
    try: t=datetime.fromisoformat(ts).replace(tzinfo=now.tzinfo)
    except: continue
    if t<now or t>now.replace(hour=now.hour,minute=0,second=0,microsecond=0)+timedelta(hours=24): continue
    try:v=float(v)
    except:continue
    age=(t-now).total_seconds()/3600
    if age<6:sums['6']+=v
    if age<12:sums['12']+=v
    if age<24:sums['24']+=v
   for h in out: sector[p[2]][h].append(sums[h])
  for h in out:
   a=sum(sector['alto'][h])/len(sector['alto'][h]) if sector['alto'][h] else None
   m=sum(sector['medio'][h])/len(sector['medio'][h]) if sector['medio'][h] else None
   if a is not None and m is not None: out[h]=(a+m)/2
   elif a is not None: out[h]=a
   elif m is not None: out[h]=m
  return out
 except Exception as e:
  print('ECMWF indisponivel:',type(e).__name__,e); return {'6':0.0,'12':0.0,'24':0.0}
...
def learn(params,snaps,history):
 ...
 for s in snaps:
  for h in ('6','12','24'):
   ...
   if not a: continue
   pred=s['pred'][h]; err=pred-a['level']; abs_err[h].append(abs(err)); total+=1
   ...
   adj('forecast_rain_gain',comps.get('forecast_rain',0),.40,1.80)
...
async def main():
 ...
 rainp=weighted_observed_rain(r['h003'],r['h006'],r['h012'],r['h024'],r['h048'],r['h096'])
 dams=get_dams(stations); dam_now=dam_signal_now(dams); future=fetch_ecmwf()
 ...
 damhist.append({'model_version':4,'t':now,'dams':dams,'signal':dam_now}); damhist=damhist[-3000:]; savej(DAM_HIST,damhist)
 snaps=loadj(SNAP,[])
 ...
 snaps.append({'t':now,'current':current,'rain':r,'future_rain':future,'dams':dam_now,'slope':slope,'pred':pred,'components':components}); snaps=snaps[-1500:]
