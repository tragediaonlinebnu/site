...
function renderFloodPerspective(){
 ...
 // Detecta janelas de precipitação e mantém pausas curtas dentro do mesmo episódio.
 const windows=[]; let cur=null;
 for(let i=0;i<pts.length;i++){
   const end=pts[i].t, start=end-5*3600000;
   const six=pts.filter(x=>x.t>=start&&x.t<=end).reduce((a,x)=>a+x.rain,0);
   if(six>=10){
     if(!cur)cur={start:pts[Math.max(0,i-5)]?.t||pts[i].t,end:pts[i].t,rain:0};
     cur.end=pts[i].t;
   }else if(cur && pts[i].t-cur.end>12*3600000){ windows.push(cur); cur=null; }
 }
 if(cur)windows.push(cur);
 windows.forEach(w=>w.rain=pts.filter(x=>x.t>=w.start&&x.t<=w.end).reduce((a,x)=>a+x.rain,0));
 ...
 // Agrupa janelas próximas. Assim, uma segunda chuva durante a recuperação continua no mesmo episódio.
 const episodes=[];
 for(const w of meaningful){
   const e=episodes[episodes.length-1];
   if(e && w.start-e.end<=36*3600000){ e.end=w.end; e.rain+=w.rain; e.windows.push(w); }
   else episodes.push({...w,windows:[w]});
 }
 const responseDelayH=Number(window.__learnedResponseDelayH)||9;
 const recoveryRateMph=0.025; // apenas para o cenário informativo; não altera a calculadora principal.
 ...
 const episodeData=episodes.map((ep,idx)=>{
   if(idx>0){
     ...
     level=Math.max(Number(blumenauRiverLevel)||0,level-recoveryRateMph*gapH);
   }
   const base=level;
