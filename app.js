...
function calculateRiverProjection(){
 ...
 for(const h of ['2','4','6','12','24']){
   ...
   const futurePart=0;
   const damSignal=Math.max(0,Math.min(1,(0.20*Number(dam.spillActive||0)+0.35*Math.max(Number(dam.eventOpening||0),Number(dam.recentEventSignal||0)))))*({6:.10,12:.20,24:.32}[h]);
...
function renderRiverCalculator(){
 ...
 <div class="river-calc-layout">
   ...
   <div class="river-calc-forecast">
     <div class="river-calc-item"><small>EM 2 HORAS</small><b>${fmt(p.p2,2)} <em>m</em></b></div>
     <div class="river-calc-item"><small>EM 4 HORAS</small><b>${fmt(p.p4,2)} <em>m</em></b></div>
