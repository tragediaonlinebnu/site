...
function mergeDamReadings(fresh){
 ...
 for(const raw of fresh||[]){
   ...
   const s={...raw,label:k==='sul'?'Barragem Sul — Ituporanga':'Barragem Oeste — Taió',subtitle:k==='sul'?'Reservatório · Rio Itajaí do Sul':'Reservatório · Rio Itajaí do Oeste',_damKey:k};
   const cur=grouped.get(k);
   if(!cur||damScore(s)>damScore(cur))grouped.set(k,s);
 }
 const out=[];
 for(const k of ['sul','oeste']){
...
async function loadTide(){
  ...
  try{
    ...
    const d=await res.json(); const events=Array.isArray(d.events)?d.events:[];
    const now=new Date();
    const upcoming=events.map(e=>({...e,dt:new Date(e.date+'T'+e.time+':00-03:00')})).filter(e=>e.dt>=now).sort((a,b)=>a.dt-b.dt);
    const next=upcoming[0]; const prev=events.map(e=>({...e,dt:new Date(e.date+'T'+e.time+':00-03:00')})).filter(e=>e.dt<now).sort((a,b)=>b.dt-a.dt)[0];
    if(!next) throw new Error('sem próximo evento');
    const kind=(i)=>i.level_m>=1?'ALTA':'BAIXA';
    const label=kind(next), status=document.getElementById('tideStatus');
    status.textContent=label; status.className='tide-status '+(label==='ALTA'?'high':'low');
    document.getElementById('tideNext').textContent=(label==='ALTA'?'⬆️ Maré alta':'⬇️ Maré baixa');
    document.getElementById('tideNextMeta').textContent=`${new Intl.DateTimeFormat('pt-BR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}).format(next.dt)}`;
    document.getElementById('tideLevel').textContent=String(next.level_m).replace('.',',');
    document.getElementById('tidePrev').textContent=prev?`${prev.time} · ${kind(prev)} · ${String(prev.level_m).replace('.',',')} m`:'—';
    const after=upcoming[1]; document.getElementById('tideAfter').textContent=after?`${after.time} · ${kind(after)} · ${String(after.level_m).replace('.',',')} m`:'—';
    document.getElementById('tideMeta').textContent=`Fonte: Defesa Civil de Itajaí / UNIVALI · atualizado ${new Date(d.updated_at).toLocaleString('pt-BR')}.`;
  }catch(e){ console.warn('Maré:',e); document.getElementById('tideStatus').textContent='INDISPONÍVEL'; document.getElementById('tideNextMeta').textContent='Aguardando atualização da fonte oficial.'; }
}
loadTide();
