...
async function loadRainForecast(){
  ...
  try{
    ...
    const now=new Date(), limits=[12,24,48,72,192].map(h=>new Date(now.getTime()+h*3600*1000));
    const vals=all.map((p,i)=>{
      ...
      for(let j=0;j<Math.min(times.length,rainv.length);j++){
        ...
        if(t>=now && t<limits[4] && Number.isFinite(v)){
    ...
    // Média independente por setor; depois, média simples entre Alto e Médio Vale.
    // Como cada setor possui o mesmo número de pontos, nenhum deles ganha peso
    // extra apenas por ter mais pontos na grade.
    ...
    const a12=basinMean(0), a24=basinMean(1), a48=basinMean(2), a72=basinMean(3), a8d=basinMean(4);
