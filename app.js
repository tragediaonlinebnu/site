...
function renderFloodPerspective(){
 ...
 const now=Date.now();
 const pts=ecmwfHourlyBasin.filter(x=>x.t>=now && Number.isFinite(x.rain)).sort((a,b)=>a.t-b.t);
 if(!pts.length){
 ...
 // Cada janela prevista é um evento independente.
 ...
 for(let i=0;i<pts.length;i++){
   ...
   if(!cur && five>=10){ cur={start:pts[Math.max(0,i-5)]?.t||t,end:t}; lastRainT=t; }
 ...
 windows.forEach(w=>{
   w.points=pts.filter(x=>x.t>=w.start&&x.t<=w.end);
   w.rain=w.points.reduce((a,x)=>a+(Number(x.rain)||0),0);
 });
 const meaningful=windows.filter(w=>w.rain>=10).sort((a,b)=>a.start-b.start);
 ...
 // Somente o evento mais próximo entra no cálculo.
 ...
 const hoursToRain=Math.max(0,(event.start-now)/3600000);
 // Este é o nível de partida do evento: nível atual + tendência média das últimas 24 h
 // projetada somente até o início da chuva.
 const levelAtRainStart=Math.max(0,baseNow+slope*hoursToRain);

 // Cada hora de chuva só passa a elevar Blumenau depois da defasagem da bacia.
 // Assim, o pico tem data/hora derivados da previsão, e não de um único horário fixo.
 const candidates=[];
 for(const p of event.points){
   const matureAt=p.t+responseDelayH*3600000;
   if(matureAt<now)continue;
   const maturedRain=event.points.filter(r=>r.t+responseDelayH*3600000<=matureAt)
     .reduce((a,r)=>a+(Number(r.rain)||0),0);
   const level=baseNow+slope*((matureAt-now)/3600000)+maturedRain*0.04;
   candidates.push({t:matureAt,level});
 }
 // Garante avaliação no fim da resposta do último pulso de chuva.
 const lastRainAt=event.points.length?event.points[event.points.length-1].t:event.end;
 const finalT=lastRainAt+responseDelayH*3600000;
 if(!candidates.some(x=>Math.abs(x.t-finalT)<60000)){
   const maturedRain=event.rain;
   candidates.push({t:finalT,level:baseNow+slope*((finalT-now)/3600000)+maturedRain*0.04});
 }
 // O nível no início da chuva também participa da busca do máximo.
 candidates.push({t:event.start,level:levelAtRainStart});
 const peak=candidates.reduce((best,x)=>x.level>best.level?x:best,candidates[0]||{t:event.start,level:levelAtRainStart});
 const estimatedPeak=Math.max(0,peak.level);
 const peakAt=new Date(peak.t);
 const likely=estimatedPeak>=4;
 status.textContent=likely?"ATENÇÃO":"POSSIBILIDADE"; status.className="flood-perspective-status "+(likely?"alert":"warn");
 body.innerHTML=`<div class="flood-perspective-summary flood-nearest-event">
   <div class="flood-perspective-main ${likely?'is-alert':''}">
     <small>PICO ESTIMADO · EVENTO MAIS PRÓXIMO</small>
   ...
   <div class="flood-perspective-metric flood-peak-date">
     ...
     <span>previsão calculada para o evento de ${floodShortDate(event.start)}–${floodShortDate(event.end)}</span>
   </div>
 </div>`;
 const trendLabel=trend.falling?"queda":trend.rising?"subida":"estável";
 meta.textContent=`Evento analisado: ${floodWindowLabel(event.start,event.end)} · ${fmt(event.rain)} mm previstos. Nível estimado no início da chuva: ${fmt(levelAtRainStart,2)} m · tendência 24 h: ${trendLabel} (${slope>=0?'+':''}${fmt(slope,3)} m/h) · resposta da bacia: ${fmt(responseDelayH,1)} h. O próximo evento será analisado separadamente quando se tornar o mais próximo.`;
}
