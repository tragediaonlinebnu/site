...
function getPanelRainAccumulated(){
 // A calculadora usa exatamente os mesmos acumulados exibidos no painel.
 // Esses valores vêm dos pluviômetros da Defesa Civil SC recebidos pelo
 // mesmo WebSocket e passam pela mesma função avg() usada nos cartões.
 const r6=avg("h006");
 const r12=avg("h012");
 const r24=avg("h024");
 const r48=avg("h048");
 return {r6,r12,r24,r48};
}

function calculateRiverProjection(){
 const current=Number(blumenauRiverLevel);
 if(!Number.isFinite(current)) return null;
 const panelRain=getPanelRainAccumulated();
 const {r6,r12,r24,r48}=panelRain;
 ...
 // A calculadora agora recebe diretamente os acumulados mostrados no
 // painel (chuva observada dos pluviômetros). O atraso de resposta continua
 // sendo tratado como parâmetro experimental, mas não há uma segunda média
 // de chuva independente dentro da calculadora.
 const responseWeight=(h,min,max)=>h<min?0.25:h<=max?1.0:0.45;
 const effectiveNow=
   Math.max(0,v6)*responseWeight(3,RESPONSE_MEDIO_MIN_H,RESPONSE_MEDIO_MAX_H)+
   Math.max(0,v12-v6)*responseWeight(9,RESPONSE_MEDIO_MIN_H,RESPONSE_MEDIO_MAX_H)+
   Math.max(0,v24-v12)*responseWeight(18,RESPONSE_MEDIO_MIN_H,RESPONSE_MEDIO_MAX_H)+
   Math.max(0,v48-v24)*responseWeight(36,RESPONSE_MEDIO_MIN_H,RESPONSE_MEDIO_MAX_H);
...
function renderRiverCalculator(){
 ...
 <div class="river-calc-note">Chuva de entrada: mesmos acumulados exibidos no painel, vindos dos pluviômetros da Defesa Civil SC via WebSocket. Estimativa experimental — não substitui os alertas oficiais da Defesa Civil.</div>`;
}
