...
let learnedParams={trend_gain:{"6":1,"12":1,"24":1},rain_gain:{"6":1,"12":1,"24":1},damping:{"6":0.70,"12":0.55,"24":0.40},metrics:{evaluated:0,mae_m:{"6":null,"12":null,"24":null}}};
async function loadLearnedParams(){
 try{
  const res=await fetch('./data/learning.json?ts='+Date.now(),{cache:'no-store'});
  if(!res.ok)return;
  const d=await res.json();
  learnedParams={...learnedParams,...d,trend_gain:{...learnedParams.trend_gain,...(d.trend_gain||{})},rain_gain:{...learnedParams.rain_gain,...(d.rain_gain||{})},damping:{...learnedParams.damping,...(d.damping||{})}};
...
function calculateRiverProjection(){
 ...
 const effectiveNow=
   v6*responseWeight(3,RESPONSE_MEDIO_MIN_H,RESPONSE_MEDIO_MAX_H)+
   (v12-v6)*responseWeight(9,RESPONSE_MEDIO_MIN_H,RESPONSE_MEDIO_MAX_H)+
   (v24-v12)*responseWeight(18,RESPONSE_MEDIO_MIN_H,RESPONSE_MEDIO_MAX_H)+
   (v48-v24)*responseWeight(36,RESPONSE_MEDIO_MIN_H,RESPONSE_MEDIO_MAX_H);
 const trend=riverTrendHistory();
 const rainPotential=Math.max(0,effectiveNow*(REFERENCE_RISE_M/REFERENCE_RAIN_MM)*0.30);
 const out={current,r24:v24,r48:v48,effective48:effectiveNow,status:projectionStatus(current),slope:trend.slope,rising:trend.rising,onsetHours:trend.onsetHours,responseWindow:'Resposta suavizada · aprendizado automático'};
 for(const h of ['6','12','24']){
   ...
   const responseFactor={6:.18,12:.38,24:.72}[h];
   const rainPart=rainPotential*responseFactor*(Number(learnedParams.rain_gain?.[h]??1));
   out['p'+h]=Math.max(0,current+trendPart+rainPart);
 }
 return out;
}
...
function renderRiverCalculator(){
 ...
 <div class="river-calc-note">Chuva de entrada: mesmos acumulados exibidos no painel, vindos dos pluviômetros da Defesa Civil SC via WebSocket. Estimativa experimental — não substitui os alertas oficiais da Defesa Civil. Aprendizado automático: ${learnCount} previsões avaliadas.</div>`;
}
