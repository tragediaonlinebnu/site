...
function renderFloodPerspective(){
 ...
 const pts=ecmwfHourlyBasin.filter(x=>x.t>=Date.now() && Number.isFinite(x.rain));
 if(!pts.length){
 ...
 // Detecta o início de uma janela pela chuva significativa, mas o acumulado
 // da janela é SEMPRE o total de todas as horas de chuva que ocorrerem até
 // 24h sem chuva. Assim, não perdemos chuva no começo/fim do período e o
 // acumulado 19→20, por exemplo, representa todo o volume desse episódio.
 const windows=[]; let cur=null; let lastRainT=null;
 for(let i=0;i<pts.length;i++){
   const t=pts[i].t, rain=Number(pts[i].rain)||0;
   const start5=t-5*3600000;
   const five=pts.filter(x=>x.t>=start5&&x.t<=t).reduce((a,x)=>a+(Number(x.rain)||0),0);
   if(!cur && five>=10){
     // Volta até 5h para capturar a chuva que iniciou o episódio.
     cur={start:pts[Math.max(0,i-5)]?.t||t,end:t,rain:0};
     lastRainT=t;
   }else if(cur && rain>0){
     // Toda chuva posterior pertence ao mesmo episódio enquanto não houver
     // 24h completas sem precipitação.
     cur.end=t;
     lastRainT=t;
   }
   if(cur && lastRainT!=null && t-lastRainT>=24*3600000){
     cur.end=lastRainT;
     windows.push(cur); cur=null; lastRainT=null;
   }
 }
 if(cur)windows.push(cur);
 windows.forEach(w=>w.rain=pts.filter(x=>x.t>=w.start&&x.t<=w.end).reduce((a,x)=>a+(Number(x.rain)||0),0));
 const meaningful=windows.filter(w=>w.rain>=10);
 if(!meaningful.length){
   status.textContent="SEM PERSPECTIVA"; status.className="flood-perspective-status ok";
   body.innerHTML='<div class="flood-perspective-empty"><strong>Sem perspectiva de cheia.</strong><br>O cenário de precipitação dos próximos 15 dias não indica elevação do nível acima de 4,00 m.</div>';
   meta.textContent=`ECMWF IFS · cenário recalculado automaticamente · última atualização ${new Date().toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"})}.`;
   return;
 }
 // A separação dos episódios não usa mais uma recuperação fixa.
 // Primeiro observamos o comportamento real do rio: se ele ainda está subindo
 // ou praticamente estável, a bacia NÃO é considerada recuperada. Se há queda
 // sustentada, usamos a própria velocidade observada de queda para estimar a
 // recuperação entre duas chuvas. Isso evita descontar 2,5 cm/h arbitrariamente.
 const episodes=[];
 const riverState=riverTrendHistory();
 const observedRecoveryRateMph=riverState.slope< -0.005
   ? Math.min(0.08,Math.max(0.005,-riverState.slope))
   : 0;
 const basinRecovering=observedRecoveryRateMph>0;
 const recoveryAmount=(peak,gapH)=>{
   if(!basinRecovering||gapH<=0)return 0;
   // A recuperação não é linear: começa próxima da velocidade observada
   // e desacelera progressivamente conforme a bacia esvazia.
   const tauH=24;
   const possible=observedRecoveryRateMph*tauH*(1-Math.exp(-gapH/tauH));
   return Math.min(Math.max(0,peak-(Number(blumenauRiverLevel)||0)),possible);
 };
 for(const w of meaningful){
   const e=episodes[episodes.length-1];
   const gapH=e?Math.max(0,(w.start-e.end)/3600000):Infinity;
   // Se ainda não há queda sustentada no rio, uma nova chuva é tratada como
   // continuidade hidrológica mesmo que a pausa entre janelas seja maior.
   if(e && (gapH<=24 || !basinRecovering)){
     e.end=w.end; e.rain+=w.rain; e.windows.push(w);
   }else episodes.push({...w,windows:[w]});
 }
 const responseDelayH=Number(window.__learnedResponseDelayH)||9;
 let level=Math.max(0,Number(blumenauRiverLevel)||0);
 const episodeData=episodes.map((ep,idx)=>{
   if(idx>0 && observedRecoveryRateMph>0){
     const gapH=Math.max(0,(ep.start-episodes[idx-1].end)/3600000);
     // Recupera apenas a parcela que pode ser explicada pela queda observada,
     // sem deixar o cenário cair artificialmente abaixo do nível atual.
     level=Math.max(Number(blumenauRiverLevel)||0,level-recoveryAmount(level,gapH));
   }
   const base=level;
   const estimatedPeak=Math.max(base,base+ep.rain*0.04);
   level=estimatedPeak;
   const onset=new Date(ep.windows[0].start+responseDelayH*3600000);
   return {...ep,base,estimatedPeak,onset};
 });
 const maxPeak=Math.max(...episodeData.map(e=>e.estimatedPeak));
 const likely=maxPeak>=4;
 const multi=episodeData.length>1;
 status.textContent=likely?"ATENÇÃO":(multi?"MÚLTIPLAS JANELAS":"POSSIBILIDADE");
 status.className="flood-perspective-status "+(likely?"alert":"warn");
 const episodeHtml=episodeData.map((ep,i)=>{
   const winHtml=ep.windows.map((w,j)=>`<span class="flood-window"><b>Janela ${j+1}</b> · ${floodWindowLabel(w.start,w.end)} · ${fmt(w.rain)} mm</span>`).join("");
   return `<div class="flood-episode ${ep.estimatedPeak>=4?'is-alert':''}">
     <div class="flood-episode-head"><strong>${multi?`Episódio ${i+1}`:'Cenário analisado'}</strong><span>${ep.estimatedPeak>=4?'acima de 4,00 m':'abaixo de 4,00 m'}</span></div>
     <div class="flood-episode-grid">
       <div><small>CHUVA PREVISTA</small><b>${fmt(ep.rain)} mm</b></div>
       <div><small>PICO ESTIMADO</small><b>${fmt(ep.estimatedPeak,2)} m</b></div>
       <div><small>POSSÍVEL INÍCIO</small><b>${floodShortDate(ep.onset)} · ${ep.onset.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}</b></div>
       <div><small>PERÍODO</small><b>${floodShortDate(ep.start)}–${floodShortDate(ep.end)}</b></div>
     </div>
     <div class="flood-perspective-windows">${winHtml}</div>
   </div>`;
 }).join("");
 body.innerHTML=`<div class="flood-perspective-summary compact">
   <div class="flood-perspective-main"><small>${likely?'MAIOR PICO ESTIMADO':'MAIOR ELEVAÇÃO ESTIMADA'}</small><strong>${fmt(maxPeak,2)} m</strong></div>
   <div class="flood-perspective-metric"><small>EPISÓDIOS</small><b>${episodeData.length}</b><span>${multi?'múltiplas janelas analisadas':'uma janela de chuva'}</span></div>
   <div class="flood-perspective-metric"><small>NÍVEL ATUAL</small><b>${fmt(blumenauRiverLevel,2)} m</b><span>leitura oficial</span></div>
 </div>
 <div class="flood-episodes-title">${multi?'Cenário com múltiplas janelas':'Cenário hidrológico'}</div>
 <div class="flood-episodes">${episodeHtml}</div>
 <div class="flood-perspective-note">As janelas próximas são analisadas em conjunto quando a bacia ainda pode estar respondendo ao episódio anterior. Novas previsões podem alterar datas, volumes e níveis estimados.</div>`;
 meta.textContent=`Segundo o modelo ECMWF IFS, este é o cenário de precipitação atualmente disponível. O boletim é recalculado a cada nova atualização da previsão e da leitura oficial do rio.`;
}
...
function renderRiverCalculator(){
 ...
 <div class="river-calc-layout">
   ...
   <div class="river-calc-forecast">
     <div class="river-calc-item"><small>EM 2 HORAS</small><b>${fmt(p2,2)} <em>m</em></b>${confidenceHtml(2)}</div>
     <div class="river-calc-item"><small>EM 4 HORAS</small><b>${fmt(p4,2)} <em>m</em></b>${confidenceHtml(4)}</div>
     <div class="river-calc-item"><small>EM 6 HORAS</small><b>${fmt(p6,2)} <em>m</em></b>${confidenceHtml(6)}</div>
     <div class="river-calc-item"><small>EM 12 HORAS</small><b>${fmt(p12,2)} <em>m</em></b>${confidenceHtml(12)}</div>
     <div class="river-calc-item"><small>EM 24 HORAS</small><b>${fmt(p24,2)} <em>m</em></b>${confidenceHtml(24)}</div>
   </div>
 </div>
