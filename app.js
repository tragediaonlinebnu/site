...
function renderFloodPerspective(){
 ...
 if(isActive && observed.rain>0){
   ...
   candidates.push({t:observedMatureAt,level:Math.max(0,baseNow+slope*(observedMatureAt-now)/3600000)+observed.rain*0.04});
 }
 ...
 if(!candidates.some(x=>Math.abs(x.t-finalT)<60000)){
   candidates.push({t:finalT,level:baseNow+slope*((finalT-now)/3600000)+totalScenarioRain*0.04});
 }
