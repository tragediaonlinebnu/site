...
// "Chuva agora" usa a média das estações selecionadas do Vale do Itajaí.
// Cada estação fornece o acumulado dos últimos 5 minutos (min005),
// convertido para mm/h equivalente.
...
function rainNow(){
 ...
 // estações que realmente registraram chuva nos últimos 5 minutos.
 const rainy=readings.filter(x=>x.value>0);
 ...
 const mean5=rainy.reduce((sum,x)=>sum+x.value,0)/rainy.length;
 return {rate:Math.max(0,mean5*12),count:readings.length,total:RAIN_NOW_STATIONS.length,stations:rainy.map(x=>x.station),active:rainy.length};
}
...
function renderRainNow(){
 ...
 meta.textContent=r.active>0
   ? `Média dos últimos 5 minutos · ${r.active} estação${r.active===1?"":"ões"} com chuva · Chuva detectada em: ${r.stations.map(s=>stationCity(s)||String(name(s)||"").replace(/^SDC-SC\s+/i, "")||"estação").join(", ")}`
   : `Últimos 5 minutos · nenhuma das 6 estações registrou chuva · Indaial, Apiúna, Timbó, Rodeio, Rio do Sul e Blumenau`;
}
