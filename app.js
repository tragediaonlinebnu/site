...
function renderHydrologyPanel(){
 ...
 note.textContent=`Modelo híbrido: tendência + chuva observada + previsão ECMWF + sinal de barragens + analogias de eventos reais. ${ev.completed_events||0} evento(s) encerrado(s) alimentam a memória histórica.`;
}
function calculateRiverProjection(){
 ...
 for(const h of ['2','4','6','12','24']){
   ...
   const forecastValue=Number(ecmwfForecastRain?.[h]||0);
   const futurePart=forecastValue*({2:0,4:0,6:.0035,12:.0055,24:.0080}[h])*(Number(learnedParams.forecast_rain_gain?.[h]??0));
   ...
   out['components'+h]={trend:trendPart,observedRain:observedPart,forecastRain:futurePart,dam:damPart,serverModel:serverFresh};
 }
 return out;
}
function renderRiverCalculator(){
 ...
 <div class="river-calc-note">Motor híbrido: nível oficial + tendência + chuva observada + previsão ECMWF quando disponível + sinais de barragens + memória de eventos históricos. A projeção exibida permanece estável até nova leitura oficial do rio. Estimativa experimental — não substitui alertas oficiais. ${hydrologyStatus?.events?.completed_events||0} eventos históricos encerrados.</div>`;
}
