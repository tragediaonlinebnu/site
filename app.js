...
let learnedParams={trend_gain:{"2":1,"4":1,"6":1,"12":1,"24":1},rain_gain:{"2":1,"4":1,"6":1,"12":1,"24":1},forecast_rain_gain:{"2":0,"4":0,"6":0,"12":0,"24":0},dam_gain:{"2":1,"4":1,"6":1,"12":1,"24":1},damping:{"2":0.88,"4":0.78,"6":0.70,"12":0.55,"24":0.40},metrics:{evaluated:0,mae_m:{"2":null,"4":null,"6":null,"12":null,"24":null}}};
...
async function loadLearnedParams(){
 try{
  ...
  if(!res.ok)return;
  const d=await res.json();
  learnedParams={...learnedParams,...d,trend_gain:{...learnedParams.trend_gain,...(d.trend_gain||{})},rain_gain:{...learnedParams.rain_gain,...(d.rain_gain||{})},forecast_rain_gain:{"2":0,"4":0,"6":0,"12":0,"24":0},dam_gain:{...learnedParams.dam_gain,...(d.dam_gain||{})},damping:{...learnedParams.damping,...(d.damping||{})}};
  renderRiverCalculator();
...
function renderRiverCalculator(){
 ...
 <div class="river-calc-note">Calculadora: nível oficial + chuva realmente observada dos pluviômetros + operação das barragens. ECMWF é apenas informativo e não entra no cálculo. A projeção exibida permanece estável até uma nova leitura oficial do rio. Estimativa experimental — não substitui os alertas oficiais da Defesa Civil. Aprendizado automático: ${learnCount} previsões avaliadas.</div>`;
}
