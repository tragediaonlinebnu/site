...
function getPanelRainAccumulated(){
 ...
 return {r3:avg("h003"),r6:avg("h006"),r12:avg("h012"),r24:avg("h024"),r48:avg("h048"),r72:avg("h072"),r96:avg("h096")};
}
function observedRainPotential(r3,r6,r12,r24,r48,r72,r96,horizon=24){
 const vals=[r3,r6,r12,r24,r48,r96].map(v=>Math.max(0,Number(v)||0));
 let [v3,v6,v12,v24,v48,v96]=vals;
 v6=Math.max(v3,v6);v12=Math.max(v6,v12);v24=Math.max(v12,v24);v48=Math.max(v24,v48);v72=Math.max(v48,v72);v96=Math.max(v72,v96);
...
let learnedParams={trend_gain:{"2":1,"4":1,"6":1,"12":1,"24":1},rain_gain:{"2":1,"4":1,"6":1,"12":1,"24":1},forecast_rain_gain:{"2":0,"4":0,"6":0,"12":0.55,"24":0.75},dam_gain:{"2":1,"4":1,"6":1,"12":1,"24":1},damping:{"2":0.88,"4":0.78,"6":0.70,"12":0.55,"24":0.40},metrics:{evaluated:0,mae_m:{"2":null,"4":null,"6":null,"12":null,"24":null}}};
...
async function loadHydrologyStatus(){
 try{
  const res=await fetch('./data/hydrology_status.json?ts='+Date.now(),{cache:'no-store'});
  if(!res.ok)return;
  hydrologyStatus=await res.json();
  renderHydrologyPanel();
  renderRiverCalculator();
 }catch(e){console.warn('Motor hidrológico',e)}
}
function renderHydrologyPanel(){
 const metrics=$('hydroMetrics'), active=$('hydroActive'), note=$('hydroNote'), badge=$('hydroModelBadge');
 if(!metrics)return;
 const ev=hydrologyStatus?.events||{};
 badge.textContent=ev.completed_events>=5?'CALIBRADO POR EVENTOS':'APRENDENDO';
 const items=[
  ['EVENTOS ENCERRADOS',ev.completed_events??0],
  ['RESPOSTA MEDIANA',ev.median_response_h==null?'—':fmt(ev.median_response_h,1)+' h'],
  ['SUBIDA / 100 mm',ev.median_rise_per_100mm_48h==null?'—':fmt(ev.median_rise_per_100mm_48h,2)+' m'],
  ['MAIOR PICO',ev.highest_peak_m==null?'—':fmt(ev.highest_peak_m,2)+' m']
 ];
 metrics.innerHTML=items.map(x=>`<div class="hydro-metric"><small>${x[0]}</small><strong>${x[1]}</strong></div>`).join('');
 const a=hydrologyStatus?.event_active;
 if(a){
   const rain=Number(a.rain_48h_last_mm); const peak=Number(a.peak_level_m);
   active.innerHTML=`<div class="hydro-active-title"><span class="hydro-pulse"></span> EVENTO EM ACOMPANHAMENTO</div><div class="hydro-active-grid"><span>Início · ${new Date(Number(a.start_t)).toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}</span><span>Nível inicial · ${fmt(Number(a.start_level_m),2)} m</span><span>Chuva 48h · ${Number.isFinite(rain)?fmt(rain,1)+' mm':'—'}</span><span>Pico atual · ${Number.isFinite(peak)?fmt(peak,2)+' m':'—'}</span></div>`;
 }else active.innerHTML='';
 note.textContent=`Modelo hidrológico: tendência + chuva média espacial observada + sinal de barragens + analogias de eventos reais. ${ev.completed_events||0} evento(s) encerrado(s) alimentam a memória histórica.`;
}
function calculateRiverProjection(){
 const current=Number(blumenauRiverLevel);
 if(!Number.isFinite(current)) return null;
 const {r3,r6,r12,r24,r48,r72,r96}=getPanelRainAccumulated();
 if(![r3,r6,r12,r24,r48,r96].some(Number.isFinite))return null;
 const trend=riverTrendHistory();
 const rainPotential={2:observedRainPotential(r3,r6,r12,r24,r48,r72,r96,2),4:observedRainPotential(r3,r6,r12,r24,r48,r72,r96,4),6:observedRainPotential(r3,r6,r12,r24,r48,r72,r96,6),12:observedRainPotential(r3,r6,r12,r24,r48,r72,r96,12),24:observedRainPotential(r3,r6,r12,r24,r48,r72,r96,24)};
 const dam=currentDamSignal();
 const out={current,r24:Number(r24)||0,r48:Number(r48)||0,r72:Number(r72)||0,r96:Number(r96)||0,effective48:rainPotential,status:projectionStatus(current),slope:trend.slope,rising:trend.rising,onsetHours:trend.onsetHours,responseWindow:'Resposta suavizada · aprendizado automático',observedRain:true,dam};
 for(const h of ['2','4','6','12','24']){
   ...
   const futurePart=0; // ECMWF é somente informativo e nunca entra no cálculo hidrológico.
   ...
   const serverPred=Number(hydrologyStatus?.predictions?.[h]);
   const serverCurrent=Number(hydrologyStatus?.current_level_m);
   const serverUpdated=hydrologyStatus?.updated_at?Date.parse(hydrologyStatus.updated_at):NaN;
   const serverFresh=Number.isFinite(serverPred)&&Number.isFinite(serverCurrent)&&Math.abs(serverCurrent-current)<0.08&&Number.isFinite(serverUpdated)&&(Date.now()-serverUpdated)<45*60*1000;
   out['p'+h]=serverFresh?Math.max(0,serverPred):Math.max(0,current+trendPart+observedPart+futurePart+damPart);
   out['components'+h]={trend:trendPart,observedRain:observedPart,forecastRain:0,dam:damPart,serverModel:serverFresh};
 }
 return out;
}
function renderRiverCalculator(){
 ...
 <div class="river-calc-layout">
   ...
   <div class="river-calc-forecast">
     <div class="river-calc-item"><small>EM 2 HORAS</small><b>${fmt(p2,2)} <em>m</em></b></div>
     <div class="river-calc-item"><small>EM 4 HORAS</small><b>${fmt(p4,2)} <em>m</em></b></div>
     <div class="river-calc-item"><small>EM 6 HORAS</small><b>${fmt(p6,2)} <em>m</em></b></div>
     <div class="river-calc-item"><small>EM 12 HORAS</small><b>${fmt(p12,2)} <em>m</em></b></div>
     <div class="river-calc-item"><small>EM 24 HORAS</small><b>${fmt(p24,2)} <em>m</em></b></div>
   </div>
 </div>
 <div class="river-calc-note">Motor hidrológico: nível oficial + tendência + chuva média espacial observada dos pluviômetros + sinais de barragens + memória de eventos históricos. A projeção exibida permanece estável até nova leitura oficial do rio. Estimativa experimental — não substitui alertas oficiais. ${hydrologyStatus?.events?.completed_events||0} eventos históricos encerrados.</div>`;
}
...
function render(){
 ...
 renderRainNow();
 renderRiverCalculator();
 const cards=[["Últimas 3h","h003"],["Últimas 6h","h006"],["Últimas 12h","h012"],["Últimas 24h","h024"],["Últimas 48h","h048"],["Últimas 72h","h072"],["Últimas 96h","__96h__"]];
...
setInterval(loadHydrologyStatus,15*60*1000);
