...
async function loadRainForecast(){
  ...
  try{
    ...
    const hourly=Array.isArray(d?.hourly_basin)?d.hourly_basin:[];
    ...
    ecmwfHourlyBasin=hourly.map(x=>({t:Number(x.t),rain:Number(x.rain)}))
      .filter(x=>Number.isFinite(x.t)&&Number.isFinite(x.rain));
    ...
    const runLabel=run&&!Number.isNaN(run.getTime())
      ? run.toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit",timeZone:"America/Sao_Paulo"})
      : "rodada ECMWF";
    const captured=d?.captured_at?new Date(d.captured_at):null;
    const capturedLabel=captured&&!Number.isNaN(captured.getTime())
      ? captured.toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit",timeZone:"America/Sao_Paulo"})
      : "";

    statusEl.textContent="ECMWF IFS · "+(d?.run_label||"RODADA");
    statusEl.className="rain-forecast-status ok";
    meta.textContent=`Média espacial uniforme · Alto Vale + Médio Vale · ${Number(d?.valid_points)||24} pontos ECMWF válidos (12+12) · rodada ${runLabel}${capturedLabel?` · capturada ${capturedLabel}`:""} · valores mantidos estáveis até a próxima rodada.`;
