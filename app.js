...
const RIVER_HISTORY_KEY="blumenau-river-history-v2";
const RIVER_HISTORY_MAX=192; // 48 h em passos de 15 min, quando a fonte permitir
...
function loadRiverHistory(){
 try{const a=JSON.parse(localStorage.getItem(RIVER_HISTORY_KEY)||"[]");return Array.isArray(a)?a.filter(x=>Number.isFinite(x?.level)&&Number.isFinite(x?.t)):[]}catch{return[]}
}
function saveRiverHistory(a){try{localStorage.setItem(RIVER_HISTORY_KEY,JSON.stringify(a.slice(-RIVER_HISTORY_MAX)))}catch{}}
function rememberRiverLevel(){
 if(!Number.isFinite(blumenauRiverLevel))return;
 const a=loadRiverHistory(), now=Date.now();
 const last=a[a.length-1];
 if(last && Math.abs(now-last.t)<5*60*1000){last.level=blumenauRiverLevel;last.updated=blumenauRiverUpdated||last.updated;saveRiverHistory(a);return}
 a.push({t:now,level:blumenauRiverLevel,updated:blumenauRiverUpdated||new Date(now).toISOString()});
 saveRiverHistory(a);
}
function riverTrendHistory(){
 ...
 let onset=null;
 if(rising){
  for(let i=recent.length-1;i>0;i--){const a1=recent[i-1],a2=recent[i];if((a2.level-a1.level)/Math.max(.25,(a2.t-a1.t)/3600000)>0.005){onset=(now-a1.t)/3600000;break}}
 }
...
function calculateRiverProjection(){
 ...
 const r6=avg("h006"), r12=avg("h012"), r24=avg("h024"), r48=avg("h048");
 if(![r6,r12,r24,r48].some(Number.isFinite))return null;
 const v6=Math.max(0,Number(r6)||0);
 const v12=Math.max(v6,Number(r12)||0);
 const v24=Math.max(v12,Number(r24)||0);
 const v48=Math.max(v24,Number(r48)||0);

 // A resposta é regional: o Médio Vale tende a repercutir mais cedo em
 // Blumenau, enquanto a contribuição do Alto Vale é tratada em uma janela
 // mais longa. São parâmetros iniciais experimentais, não tempos oficiais.
 const medioCities=MEDIO_VALE;
 const altoCities=ALTO_VALE;
 const m6=Math.max(0,Number(avgForCities("h006",medioCities))||0);
 const m12=Math.max(m6,Number(avgForCities("h012",medioCities))||0);
 const m24=Math.max(m12,Number(avgForCities("h024",medioCities))||0);
 const m48=Math.max(m24,Number(avgForCities("h048",medioCities))||0);
 const a6=Math.max(0,Number(avgForCities("h006",altoCities))||0);
 const a12=Math.max(a6,Number(avgForCities("h012",altoCities))||0);
 const a24=Math.max(a12,Number(avgForCities("h024",altoCities))||0);
 const a48=Math.max(a24,Number(avgForCities("h048",altoCities))||0);

 const responseWeight=(h,min,max)=>h<min?0.25:h<=max?1.0:0.45;
 const effectiveMedio=
   Math.max(0,m6)*responseWeight(3,RESPONSE_MEDIO_MIN_H,RESPONSE_MEDIO_MAX_H)+
   Math.max(0,m12-m6)*responseWeight(9,RESPONSE_MEDIO_MIN_H,RESPONSE_MEDIO_MAX_H)+
   Math.max(0,m24-m12)*responseWeight(18,RESPONSE_MEDIO_MIN_H,RESPONSE_MEDIO_MAX_H)+
   Math.max(0,m48-m24)*responseWeight(36,RESPONSE_MEDIO_MIN_H,RESPONSE_MEDIO_MAX_H);
 const effectiveAlto=
   Math.max(0,a6)*responseWeight(3,RESPONSE_ALTO_MIN_H,RESPONSE_ALTO_MAX_H)+
   Math.max(0,a12-a6)*responseWeight(9,RESPONSE_ALTO_MIN_H,RESPONSE_ALTO_MAX_H)+
   Math.max(0,a24-a12)*responseWeight(18,RESPONSE_ALTO_MIN_H,RESPONSE_ALTO_MAX_H)+
   Math.max(0,a48-a24)*responseWeight(36,RESPONSE_ALTO_MIN_H,RESPONSE_ALTO_MAX_H);

 // Mantemos uma parcela da média geral para evitar descontinuidade quando
 // alguma região não tiver estação válida no ciclo atual.
 const hasMedio=Number.isFinite(avgForCities("h024",medioCities));
 const hasAlto=Number.isFinite(avgForCities("h024",altoCities));
 let effectiveNow;
 if(hasMedio && hasAlto) effectiveNow=(effectiveMedio+effectiveAlto)/2;
 else if(hasMedio) effectiveNow=effectiveMedio;
 else if(hasAlto) effectiveNow=effectiveAlto;
 else {
   const rain0_6=v6, rain6_12=Math.max(0,v12-v6), rain12_24=Math.max(0,v24-v12), rain24_48=Math.max(0,v48-v24);
   effectiveNow=rain0_6*0.35+rain6_12*0.35+rain12_24*0.55+rain24_48*0.45;
 }

 const trend=riverTrendHistory();
 // A referência histórica 130 mm -> +5,90 m é apenas uma escala inicial.
 // O modelo não transforma esse evento em uma regra fixa: ele combina chuva
 // regional, atraso de resposta e tendência real observada em Blumenau.
 const rainPotential=Math.max(0,effectiveNow*(REFERENCE_RISE_M/REFERENCE_RAIN_MM));
 const trend6=Math.max(-0.30,Math.min(0.60,trend.slope*6));
 const trend12=Math.max(-0.45,Math.min(1.00,trend.slope*12));
 const trend24=Math.max(-0.60,Math.min(1.40,trend.slope*24));
 const response6=rainPotential*0.18;
 const response12=rainPotential*0.38;
 const response24=rainPotential*0.72;
 const p6=Math.max(0,current+trend6+response6);
 const p12=Math.max(0,current+trend12+response12);
 const p24=Math.max(0,current+trend24+response24);
 return {
   current,r24:v24,r48:v48,effective48:effectiveNow,p6,p12,p24,
   status:projectionStatus(p12),slope:trend.slope,rising:trend.rising,onsetHours:trend.onsetHours,
   responseWindow:`Médio ${RESPONSE_MEDIO_MIN_H}–${RESPONSE_MEDIO_MAX_H} h · Alto ${RESPONSE_ALTO_MIN_H}–${RESPONSE_ALTO_MAX_H} h`
 };
}
function renderRiverCalculator(){
 ...
 host.innerHTML=`
 <div class="river-calc-head"><div><div class="river-calc-kicker">PROJEÇÃO DO RIO · BLUMENAU</div><h3>Nível do Itajaí-Açu</h3><p>Leitura oficial atual + estimativa para as próximas horas.</p></div><span class="river-calc-badge ${status.cls}">${status.label}</span></div>
 ...
 <div class="river-calc-note">Estimativa experimental — não substitui os alertas oficiais da Defesa Civil.</div>`;
}
